defmodule SmoothieApi.Web.Lib.EctoTypeUtils do
  defmacro __using__(_env) do
    quote do
      import SmoothieApi.Web.Lib.EctoTypeUtils
    end
  end
  ...
  defmacro def_ecto_types(model_name, attrs \\ []) do
    ...
    quote do
      ...
      def changeset(struct, params \\ %{}),
        do:
          SmoothieApi.Web.Lib.EctoUtils.changeset_in(struct, params, @fields, @validated_fields)
        ...
        Module.eval_quoted(
          ...
          quote do
            ...
            def unquote(pluralize(field_name))(ids, options),
              do: SmoothieApi.Web.Lib.EctoUtils.ids_in(MODEL, unquote(field_name), ids, options)

            @spec unquote(field_name)(String.t(), keyword()) :: MODEL.t() | nil
            def unquote(field_name)(id, options \\ []),
              do: SmoothieApi.Web.Lib.EctoUtils.id_in(MODEL, unquote(field_name), id, options)
          end
        )
      end)

      @spec all(keyword()) :: %{String.t() => Model.t()}
      def all(options), do: SmoothieApi.Web.Lib.EctoUtils.all_in(MODEL, options)

      defmodule Multi do
        @spec insert(Ecto.Multi.t(), MODEL.t(), keyword()) :: Ecto.Multi.t()
        def insert(multi, struct, options \\ []),
          do: SmoothieApi.Web.Lib.EctoUtils.Multi.insert_in(MODEL, multi, struct, options)

        @spec delete(Ecto.Multi.t(), MODEL.t(), keyword()) :: Ecto.Multi.t()
        def delete(multi, struct, options \\ []),
          do: SmoothieApi.Web.Lib.EctoUtils.Multi.delete_in(MODEL, multi, struct, options)

        @spec update(Ecto.Multi.t(), MODEL.t(), MODEL.input(), keyword()) :: Ecto.Multi.t()
        def update(multi, struct, params, options \\ []),
          do: SmoothieApi.Web.Lib.EctoUtils.Multi.update_in(MODEL, multi, struct, params, options)

        @spec upsert(Ecto.Multi.t(), MODEL.t(), MODEL.t(), keyword()) :: Ecto.Multi.t()
        def upsert(multi, struct, params, options \\ []),
          do: SmoothieApi.Web.Lib.EctoUtils.Multi.upsert_in(MODEL, multi, struct, params, options)
      end
    end
  end
end
